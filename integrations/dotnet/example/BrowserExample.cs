...
// This example demonstrates using js65.browser in a .NET 8+ browser (WASM) environment.
// Note: This requires the JavaScript runtime to be initialized before use. See main.js
// for an example of how to use this.

public partial class BrowserExample
{
    [JSExport]
    public static async Task<string> RunExample()
    {
        ...
        // The JsFileCallbackConfig tells the engine where to find JS functions for loading include files.
        var asm = new BrowserJsEngine(
            fileCallbackConfig: new JsFileCallbackConfig
            {
                ModulePath = "/main.js",  // Absolute path from app root
                // Uses default function names: readFileAsTextSync, readFileAsBinarySync
            })
        {
            Options = {
                includePaths = ["./"],
...
""", "test_include.s");
        ...
        // BrowserJsEngine uses the native browser JavaScript engine via JSInterop, so it's lightweight and doesn't require
        // bundling a separate JS runtime. However, it requires running in a browser WASM context.
        ...
        output.AppendLine("Test patch to call the new armor subtract function");
        output.AppendLine($"sbc,x instruction should be patched to jsr ($20): ${result?.romdata[0x10 + 0x0005]:x2}");
        output.AppendLine($"included file should write constant ($89) at address $8089: ${result?.romdata[0x10 + 0x0089]:x2}");
