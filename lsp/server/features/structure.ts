...
/**
 * Structural features that re-lex the open document with `Tokenizer` directly,
 * without running the full assemble pipeline. This is the one place a raw
 * re-lex is correct: the features must work on broken, half-typed text where
 * the full pipeline would bail, and they don't need symbol resolution.
 *
 *   - `textDocument/foldingRange`: open/close pairs of `.scope`/`.endscope`,
 *     `.proc`/`.endproc`, `.macro`/`.endmacro`, `.repeat`/`.endrep`.
 *   - `textDocument/semanticTokens/full`: token-class-based highlighting.
 */
...
export function registerStructureFeatures(connection: Connection, analyzer: Analyzer): void {
  connection.onFoldingRanges((p): FoldingRange[] => {
    const text = analyzer.peekDoc(p.textDocument.uri);
    if (text == null) return [];
    return computeFolding(text);
  });

  connection.languages.semanticTokens.on((p): SemanticTokens => {
    const text = analyzer.peekDoc(p.textDocument.uri);
    if (text == null) return {data: []};
    return computeSemanticTokens(text);
  });

  // Many clients call `semanticTokens/full`; ensure we registered the
  // provider-type capability on initialize so they pick us up.
}
 ...
 */
function computeFolding(text: string): FoldingRange[] {
...
/**
 * Re-lex the document into one entry per logical line, carrying the real
 * 0-based source line off `token.source`.
 *
 * The array index is *not* the source line: `nextSync()` skips leading EOLs, so
 * blank and comment-only lines never produce an entry. Using the index displaces
 * every fold in a commented file by the number of preceding blank lines, which
 * is why this lexes with `generateDebugInfo: true` and reads the line back.
 */
 ...
 */
function computeSemanticTokens(text: string): SemanticTokens {
...
// Re-export for tests.
export {computeFolding, computeSemanticTokens};
